---
updated_at: 2026-04-08T19:21:00.390+10:00
---
...
4. When everyone has had a turn, the combatant with the highest initiative total acts again, and steps 3 and 4 repeat until combat ends.
...
If the combatants are all unprepared for combat, then each rolls 2D6 and adds their Dexterity DM to determine starting Initiative. Initiative determines the order that characters act in, but it can also be spent to react to events. If some of the combatants are ready for combat and some are not, such as in an ambush, the prepared characters are considered to get an automatic 12 on their roll, giving them an Initiative of 12 + Dexterity DM. Characters that are aware of a group, and that group is unaware of them, may elect to avoid conflict entirely. Characters that have the Tactics skill may make a Tactics check and add the Effect of this check to the Initiative of everyone in their unit with whom they are in direct communication.
...
The most common significant action is an attack. The basic attack action is trying to injure a foe with a melee attack or a ranged weapon. The attacker declares his target, and the foe may choose to react. The attacker then makes a skill check, and if successful, deals damage to his target. The actual Difficulty of this skill check is determined using the weapon type and the range of the attack, as given earlier in this chapter under Range. The standard skill checks used in making an attack are:
...
the description mentions otherwise up to two people can fire into each arc from a civilian vehicle and one person in each arc in a military one.
...
has woven into an obstacle and crashed. If he succeeds, any pursuers must choose a weave action on their turn and make their skill test at the same penalty with the same consequence for failure. Alternatively, they can choose to break off pursuit and either give up or try to reacquire the target later.
...
---
Дано: 
- Сервер (без графического окружения) с читым Ubuntu 20.04 и доступом к репозиторию. 
- К серверу можно подключиться через ssh соединение.
- На сервере есть права sudo и root.
Задача: Сделай подробную и пошаговую инструкцию для того чтобы 
- установить postgresql
- создать и запустить базу
- получить доступ для внутреннего консольного клиента.
- получить доступ для внешнего клиента (pgadmin) с windows машины.
Условия:
- Инструкция должна содержать подробное объяснение каждого шага: его причин и целей
- Каждая команда должна объясняться: что она делает? что делают её ключи и почему выбраны именно эти значения
- Инструкция должна быть написана сухим техническим языком и не содержать эмоциональных оценок
- Если нет однозначного действия в конкретном шаге - должны быть описаны все варианты.
Назначение: Инструкция будет помещена в базу знаний для сотрудников. Кней будут обращаться как молодые-неопытные сотрудники, так и старшие разработчики из других отделов.
Органичения: если для написания инструкции требуется что-то предположить, немедленно прекрати работу над инструкцией и задай вопросы требующие уточнения.
