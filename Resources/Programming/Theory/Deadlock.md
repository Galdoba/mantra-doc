---
updated_at: 2026-02-12T16:48:32.240+10:00
---
...
1.  **Необходимые условия ([[Coffman Conditions|условия Коффмана]])**. Для возникновения взаимной блокировки должны одновременно выполняться четыре условия:
    *   **[[Mutual Exclusion|взаимное исключение]]** — ресурс не может быть одновременно использован несколькими процессами;
    *   **[[Hold and Wait|удержание и ожидание]]** — процесс удерживает выделенный ресурс и ожидает получения дополнительных;
    *   **[[No Preemption|отсутствие принудительного освобождения]]** — ресурс может быть освобождён только самим удерживающим процессом;
    *   **[[Circular Wait|циклическое ожидание]]** — существует замкнутая цепочка процессов, каждый из которых ожидает ресурс, удерживаемый следующим в цепи.
...
3.  **Сложность обнаружения**. Алгоритмы обнаружения взаимных блокировок требуют построения и анализа [[Resource Allocation Graph|графа распределения ресурсов]], что вычислительно затратно и должно выполняться периодически.
...
1.  **Предотвращение ([[Deadlock Prevention|deadlock prevention]])**. Устранение одного из условий Коффмана:
    *   **Упорядоченный захват ресурсов ([[Lock Ordering|lock ordering]])** — все процессы захватывают ресурсы в едином, глобально определённом порядке. Это устраняет циклическое ожидание.
...
2.  **Обход ([[Deadlock Avoidance|deadlock avoidance]])**. Динамическая оценка безопасности каждого запроса ресурса. Система проверяет, приведёт ли удовлетворение запроса к возможной взаимной блокировке в будущем. Классический алгоритм — [[Banker's Algorithm|алгоритм банкира]] (Дейкстра). Требует априорного знания максимальной потребности каждого процесса в ресурсах.

3.  **Обнаружение и восстановление ([[Deadlock Detection and Recovery|deadlock detection and recovery]])**:
    *   **Периодическое построение [[Resource Allocation Graph|графа распределения ресурсов]]** и поиск циклов. При обнаружении deadlock выполняется восстановление.
    ...
    *   **Вытеснение ресурса ([[Resource Preemption|resource preemption]])** — принудительное изъятие ресурса у процесса и передача его другому; требует сохранения состояния для последующего восстановления.

4.  **Игнорирование ([[Ostrich Algorithm|страусиный алгоритм]])**. Предположение, что взаимные блокировки происходят крайне редко, и затраты на их предотвращение или обнаружение не оправданы. Распространено в пользовательских приложениях, где deadlock обычно воспринимается как зависание и устраняется перезапуском.

5.  **Использование таймаутов ([[Lock Timeout|lock timeout]])**. При захвате ресурса указывается максимальное время ожидания. По истечении таймаута запрос прерывается, ресурсы освобождаются, выполняется повторная попытка. Это не предотвращает deadlock, но позволяет из него выйти.

6.  **Иерархия блокировок ([[Lock Hierarchy|lock hierarchy]])**. В сложных системах вводится числовой уровень для каждого ресурса; поток может захватывать ресурсы только с монотонно возрастающими уровнями. Нарушение порядка вызывает исключение.
...
8.  **Проектирование без блокировок ([[Lock-Free Programming|lock-free programming]])**. Использование атомарных операций и неблокирующих алгоритмов, полностью исключающих необходимость во взаимных исключениях. Наиболее радикальный, но и наиболее сложный подход.
