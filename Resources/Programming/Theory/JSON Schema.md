---
updated_at: 2026-02-16T13:06:49.604+10:00
---
...
5.  **Дробление версий (draft)**. Существует несколько активно используемых версий спецификации (draft-04, draft-06, draft-07, 2020-12), не полностью совместимых между собой. Выбор версии влияет на доступные ключевые слова и поведение валидатора.
...
2.  **Модульность и переиспользование (`$ref`)**. Повторяющиеся части схемы выносятся в отдельные определения (`$defs` или `definitions`) и ссылаются на них через `$ref`. Это сокращает дублирование и упрощает поддержку.
...
5.  **Условная валидация (conditional validation)**. Ключевые слова `if`, `then`, `else` позволяют изменять требования в зависимости от значений других полей. Например, если поле `status` равно `"active"`, то поле `endDate` должно отсутствовать.
