...
1.  **[[Repository (pattern)|Репозиторий]] с методами поиска**. Интерфейс репозитория предоставляет специализированные методы для типовых запросов (`findById`, `findByStatus`, `findAllByCustomerId`). Это инкапсулирует логику доступа к данным и повышает читаемость кода.
