---
updated_at: 2026-02-18T12:11:26.185+10:00
---
...
*   **Централизация сквозных функций**. Аутентификация, авторизация, ограничение частоты запросов ([Rate Limiting]), сбор метрик и логирование выносятся в шлюз, освобождая от их реализации в каждом сервисе.
...
*   **Безопасность**. Шлюз скрывает внутреннюю структуру системы от внешнего мира, защищает от атак ([DDoS]) и централизует политики безопасности.
...
2.  **Увеличение задержки ([Latency])**. Каждый запрос проходит дополнительный сетевой прыжок, что добавляет накладные расходы. В высоконагруженных системах это может быть критично.
...
2.  **Серверная часть для конкретного интерфейса ([Backend for Frontend (BFF)]**. Для каждого типа клиентов (мобильное приложение, веб, сторонние API) создаётся отдельный шлюз, оптимизированный под их специфику. Это предотвращает распухание единого шлюза и позволяет независимо эволюционировать клиентским интерфейсам.
...
4.  **Использование sidecar-прокси и сервисной сетки**. Внутренняя маршрутизация и сквозные функции между сервисами часто реализуются на уровне sidecar-прокси (Envoy, Linkerd, Istio), что освобождает API-шлюз от ответственности за межсервисное взаимодействие.
...
6.  **Поддержка GraphQL и агрегация**. Шлюз может выступать единой точкой для GraphQL-запросов, агрегируя данные из множества нижележащих REST или gRPC-сервисов.

7.  **Безопасность на периметре**. Шлюз реализует:
    *   терминирование TLS ([TLS Termination]);
